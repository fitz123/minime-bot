...
RESTART_SUPERVISOR_PLIST="${RESTART_SUPERVISOR_PLIST:-}"
...
ensure_restart_paths() {
  ...
  if [ -z "$RESTART_SUPERVISOR_PLIST" ]; then
    RESTART_SUPERVISOR_PLIST="$RESTART_RUNTIME_DIR/${RESTART_REQUEST_ID}.${RESTART_SUPERVISOR_LABEL}.plist"
  fi
}
