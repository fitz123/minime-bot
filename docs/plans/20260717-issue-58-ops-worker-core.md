...
- [ ] Integrate a `worker` command group into `src/cli.ts` with the minimum PR-1 surfaces needed to start the local supervisor and inspect/submit/retry/cancel core tasks. Inputs select only registered task templates, authorization profiles, and done checks; no arbitrary command/URL fields.
- [ ] Add the smallest loopback health/status endpoint skeleton only; no HTTP task-intake route in PR 1 — authenticated intake is PR 2, and PR-1 task submission happens only via the CLI. The package must never start the worker merely because the bot starts or the package is installed.
- [ ] Add concise `docs/ops-worker.md` documentation that labels Alertmanager/reporting/production rollout as PR 2 or later; add example fixtures only if a PR-1 test consumes them, placeholders only.
- [ ] Add metrics/state inspection only where required to test the core; defer production telemetry/report transport and the 24-case fault lab to PR 2.
