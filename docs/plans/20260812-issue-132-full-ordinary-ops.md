...
Everything else is rejected, explicitly including raw `bindings`, behavioral binding fields, `agents.<id>.workspaceCwd`, `agents.<id>.model`, `agents.<id>.thinking`, `agents.<id>.systemPrompt`, `agents.<id>.askAgent`, `sessionDefaults`, `piExtraExtensions`, `discord`, `logLevel`, and any unknown key. Both deployments keep the same resolved canonical `workspaceCwd`. Their bot session stores stay under their distinct control roots, Pi transcripts use distinct existing `PI_CODING_AGENT_SESSION_DIR` values, and `src/pi-rpc-protocol.ts` passes the deployment control root as `artifactWorkspaceCwd` so generated context/persona artifacts land in that root's existing `.tmp` runtime directory rather than the shared live context tree.
...
- [ ] in `src/config.ts`, load the instance layer after `config.local.yaml`; allow only the scalar/object paths listed in Technical approach plus `bindingIdentityOverrides.<canonical-label>.chatId`/`.topicId`, require each label key to match exactly one canonical binding, and reject raw `bindings`, every behavioral binding field, `agents.<id>.workspaceCwd`, every other non-allowlisted key path, missing labels, and duplicate canonical labels with errors that name no value
- [ ] apply each identity patch without replacing its canonical binding, proving that `agentId`, `kind`, `label`, `requireMention`, `topics`, `voiceTranscriptEcho`, `typingIndicator`, and all unmentioned fields remain canonical
- [ ] in `src/pi-rpc-protocol.ts`, keep the canonical `agent.workspaceCwd` as the context source and Pi cwd but pass `resolveWorkspaceContract().paths.controlWorkspaceRoot` to `assemblePiContext` as `artifactWorkspaceCwd`, so generated context/persona files use the deployment's existing `.tmp` runtime directory; retain `PI_CODING_AGENT_SESSION_DIR` as the existing per-deployment Pi transcript selector
...
- [ ] write tests in `src/__tests__/workspace-contract.test.ts`, `src/__tests__/workspace-validator.test.ts`, `src/__tests__/config-merge.test.ts`, and `src/__tests__/config-defaults.test.ts` for path defaults/precedence, instance precedence, every allowed leaf, identity-only patch preservation, unique-label failures, and rejection of raw/behavioral binding fields, `agents.<id>.workspaceCwd`, model/thinking/session/extension/Discord/unknown overrides without value leakage
...
- [ ] update `src/__tests__/context-assembler.test.ts`, `src/__tests__/pi-rpc-protocol.test.ts`, and `src/__tests__/interactive-session-binding-integration.test.ts` to prove both deployments read one canonical live context while their context artifacts, `PI_CODING_AGENT_SESSION_DIR` transcripts, and control-root bot session-store paths are distinct
- [ ] run `npm run lint`, then `npm run test:file -- src/__tests__/workspace-contract.test.ts src/__tests__/workspace-validator.test.ts src/__tests__/config-merge.test.ts src/__tests__/config-defaults.test.ts src/__tests__/media-store.test.ts src/__tests__/echo-watcher.test.ts src/__tests__/context-assembler.test.ts src/__tests__/pi-rpc-protocol.test.ts src/__tests__/interactive-session-binding-integration.test.ts`, then the canonical full package gate — all must pass before Task 2
...
- [ ] run `npm run lint`, then `npm run test:file -- src/__tests__/project-naming.test.ts src/__tests__/package-install.test.ts src/__tests__/workspace-contract.test.ts src/__tests__/config-merge.test.ts src/__tests__/pi-rpc-protocol.test.ts src/__tests__/interactive-session-binding-integration.test.ts src/__tests__/trigger-input.test.ts src/__tests__/monitoring-native.test.ts`, then `python3 -m unittest discover -s scripts/tests`, then the canonical full package gate; run `git diff --check` and report changed files, commands, and results
...
- Coexistence check: Primary and Ops serve their own DMs and voice concurrently, read the same live context, keep Pi transcripts/context artifacts/bot sessions/media/echo/runtime state isolated, and cannot prune each other's artifacts.
